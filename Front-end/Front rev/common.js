...
const CONFIG = {
  ...
  API_BASE_URL: '/api', // Base URL para APIs (será configurada pelo backend)
  ITEMS_PER_PAGE: 10
};
...
const Auth = {
  ...
  getUserInfo() {
    return {
      username: sessionStorage.getItem('usuario_logado'),
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!window.location.pathname.includes('index.html') && 
    ...
    if (usernameDisplay || loginTimeDisplay) {
      ...
      if (usernameDisplay) {
        usernameDisplay.textContent = userInfo.username || 'Usuário';
      }
